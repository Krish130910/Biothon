...
async function runTestRunner() {
  ...
  console.log("Profile A (Clinical Overall Risk):", clinicalA.overallRisk, `(${clinicalA.overallRiskLabel})`);
  console.log("Profile A (ML Risk Category):   ", mlA.mlRiskCategory, `(Confidence: ${Math.round(mlA.confidence * 100)}%, Version: ${mlA.modelVersion})`);
  ...
  console.log("\nProfile B (Clinical Overall Risk):", clinicalB.overallRisk, `(${clinicalB.overallRiskLabel})`);
  console.log("Profile B (ML Risk Category):   ", mlB.mlRiskCategory, `(Confidence: ${Math.round(mlB.confidence * 100)}%, Version: ${mlB.modelVersion})`);
  ...
  console.log("\nProfile C (Clinical Overall Risk):", clinicalC.overallRisk, `(${clinicalC.overallRiskLabel})`);
  console.log("Profile C (ML Risk Category):   ", mlC.mlRiskCategory, `(Confidence: ${Math.round(mlC.confidence * 100)}%, Version: ${mlC.modelVersion})`);
  ...
  const sugarA = FoodImpactService.analyzePersonalizedFood(highSugarFood.ingredients, highSugarFood.nutritionFacts, getPersonalizedRisks(clinicalA));
  const sugarB = FoodImpactService.analyzePersonalizedFood(highSugarFood.ingredients, highSugarFood.nutritionFacts, getPersonalizedRisks(clinicalB));
  console.log("Profile A (Low Diabetes Risk) -> Score:", sugarA.personalizedFoodScore, "Category:", sugarA.foodRiskCategory, "Sugar/Diabetes Impact:", sugarA.diabetesImpact);
  console.log("Profile B (High Diabetes Risk) -> Score:", sugarB.personalizedFoodScore, "Category:", sugarB.foodRiskCategory, "Sugar/Diabetes Impact:", sugarB.diabetesImpact);

  // Food Sodium Impact scaling
  console.log("\n--- High-Sodium Food Assessment ---");
  const sodiumA = FoodImpactService.analyzePersonalizedFood(highSodiumFood.ingredients, highSodiumFood.nutritionFacts, getPersonalizedRisks(clinicalA));
  const sodiumC = FoodImpactService.analyzePersonalizedFood(highSodiumFood.ingredients, highSodiumFood.nutritionFacts, getPersonalizedRisks(clinicalC));
  console.log("Profile A (Low HT Risk)  -> Score:", sodiumA.personalizedFoodScore, "Category:", sodiumA.foodRiskCategory, "Sodium/HT Impact:", sodiumA.hypertensionImpact);
  console.log("Profile C (High HT Risk) -> Score:", sodiumC.personalizedFoodScore, "Category:", sodiumC.foodRiskCategory, "Sodium/HT Impact:", sodiumC.hypertensionImpact);

  // Healthy Food Assessment
  console.log("\n--- Healthy Food Assessment ---");
  const cleanA = FoodImpactService.analyzePersonalizedFood(healthyFood.ingredients, healthyFood.nutritionFacts, getPersonalizedRisks(clinicalA));
  const cleanC = FoodImpactService.analyzePersonalizedFood(healthyFood.ingredients, healthyFood.nutritionFacts, getPersonalizedRisks(clinicalC));
  console.log("Profile A (Clean Food) -> Score:", cleanA.personalizedFoodScore, "Category:", cleanA.foodRiskCategory);
  console.log("Profile C (Clean Food) -> Score:", cleanC.personalizedFoodScore, "Category:", cleanC.foodRiskCategory);
